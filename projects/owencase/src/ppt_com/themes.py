...
class SetThemeColorsInput(BaseModel):
    ...
    def require_explicit_color(self):
        if not any(value is not None for value in self.model_dump().values()):
            raise ValueError("Specify at least one exact theme color to change")
        return self
...
def set_theme_colors(params: SetThemeColorsInput) -> str:
    ...
    try:
        ...
        color_map = {
            THEME_COLOR_MAP[name]: hex_to_int(hex_val)
            for name, hex_val in explicit.items()
        }
...
def register_tools(mcp):
    ...
    async def tool_set_theme_colors(params: SetThemeColorsInput) -> str:
        ...
        Specify only the exact color slots requested by the user. No preset,
        generated palette, or fallback colors are applied. Omitted colors remain
        unchanged on every slide master.
        """
