...
_DOWNLOAD_TIMEOUT_SECONDS = float(os.getenv("PPT_DOWNLOAD_TIMEOUT_SECONDS", "15"))
_MAX_DOWNLOAD_BYTES = int(os.getenv("PPT_MAX_DOWNLOAD_BYTES", str(20 * 1024 * 1024)))
...
def _validate_remote_url(url: str) -> None:
    ...
    for address in addresses:
        ...
        if (
            ...
            raise ValueError(f"URL resolves to a blocked network address: {address}")
...
def _download_url(url: str) -> tuple[bytes, str]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "ppt-mcp-bob/1"})
    with opener.open(request, timeout=_DOWNLOAD_TIMEOUT_SECONDS) as response:
        ...
        if content_length and int(content_length) > _MAX_DOWNLOAD_BYTES:
            raise ValueError(
                f"Download exceeds PPT_MAX_DOWNLOAD_BYTES ({_MAX_DOWNLOAD_BYTES})"
            )
...
def _fetch_icon_metadata():
    ...
    data, _ = _download_url(_ICON_METADATA_URL)
...
class AddSvgIconInput(BaseModel):
    ...
    color: str = Field(
        ...
        description=(
            ...
            "(e.g. 'accent1', 'accent2', 'dark1', 'light1'). No fallback "
            "color is applied."
        ),
    )
    style: str = Field(
...
def _set_default_fonts_impl(latin, east_asian, apply_to_existing):
    ...
    try:
        ...
        count = designs.Count
        if count < 1:
            raise RuntimeError("Presentation has no design/slide-master collection")
...
def set_tag(params: SetTagInput) -> str:
    ...
    """
    return execute_json('Failed to set tag: ',
            _set_tag_impl,
...
def get_tags(params: GetTagsInput) -> str:
    ...
    """
    return execute_json('Failed to get tags: ',
            _get_tags_impl,
...
def replace_font(params: ReplaceFontInput) -> str:
    ...
    """
    return execute_json('Failed to replace font: ',
            _replace_font_impl,
...
def list_fonts() -> str:
    ...
    """
    return execute_json('Failed to list fonts: ', _list_fonts_impl)
...
def set_default_fonts(params: SetDefaultFontsInput) -> str:
    ...
    """
    return execute_json('Failed to set default fonts: ',
...
def crop_picture(params: CropPictureInput) -> str:
    ...
    """
    return execute_json('Failed to crop picture: ',
            _crop_picture_impl,
            params.slide_index, params.shape_name_or_index,
...
def set_picture_format(params: SetPictureFormatInput) -> str:
    ...
    """
    return execute_json('Failed to set picture format: ',
...
def set_slide_hidden(params: SetSlideHiddenInput) -> str:
    ...
    """
    return execute_json('Failed to set slide hidden: ',
...
def get_selection() -> str:
    ...
    """
    return execute_json('Failed to get selection: ', _get_selection_impl)
...
def set_view(params: SetViewInput) -> str:
    ...
    """
    return execute_json('Failed to set view: ',
            _set_view_impl,
...
def copy_animation(params: CopyAnimationInput) -> str:
    ...
    """
    return execute_json('Failed to copy animation: ',
...
def add_picture_from_url(params: AddPictureFromUrlInput) -> str:
    ...
    """
    return execute_json('Failed to add picture from URL: ',
...
def add_svg_icon(params: AddSvgIconInput) -> str:
    ...
    """
    return execute_json('Failed to add SVG icon: ',
            _add_svg_icon_impl,
...
def lock_aspect_ratio(params: LockAspectRatioInput) -> str:
    ...
    """
    return execute_json('Failed to lock aspect ratio: ',
...
def set_default_shape_style(params: SetDefaultShapeStyleInput) -> str:
    """Set the presentation's default style for newly created shapes."""
