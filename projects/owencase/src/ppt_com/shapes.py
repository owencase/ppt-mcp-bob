...
class ShapeIdentifierInput(BaseModel):
    """Input for identifying a shape by stable ID, name, or index."""
    model_config = ConfigDict(str_strip_whitespace=True)

    slide_index: int = Field(..., ge=1, description="1-based slide index")
    shape_id: Optional[int] = Field(default=None, ge=1, description="Stable PowerPoint Shape.ID (preferred)")
    ...
    def require_one_identifier(self):
        supplied = sum(
            value is not None
            for value in (self.shape_id, self.shape_name, self.shape_index)
        )
        if supplied != 1:
            raise ValueError("Provide exactly one of shape_id, shape_name, or shape_index")
        return self
...
def _get_shape_info_impl(slide_index, shape_id, shape_name, shape_index):
    app = ppt._get_app_impl()
    pres = ppt._get_pres_impl()
    slide = pres.Slides(slide_index)
    shape = _get_shape(
        slide, None, shape_name=shape_name, shape_index=shape_index, shape_id=shape_id
    )
...
def _update_shape_impl(slide_index, shape_id, shape_name, shape_index, left, top, width, height, rotation, name, adjustments):
    app, pres, slide = get_slide(slide_index)
    shape = _get_shape(
        slide, None, shape_name=shape_name, shape_index=shape_index, shape_id=shape_id
    )
...
def _delete_shape_impl(slide_index, shape_id, shape_name, shape_index):
    app, pres, slide = get_slide(slide_index)
    shape = _get_shape(
        slide, None, shape_name=shape_name, shape_index=shape_index, shape_id=shape_id
    )
    deleted_name = shape.Name
    deleted_id = int(shape.Id)
    shape.Delete()
    return {
        "success": True,
        "deleted_shape_id": deleted_id,
        "deleted_shape_name": deleted_name,
    }


def _duplicate_shape_impl(slide_index, shape_id, shape_name, shape_index):
    app, pres, slide = get_slide(slide_index)
    shape = _get_shape(
        slide, None, shape_name=shape_name, shape_index=shape_index, shape_id=shape_id
    )
    ...
    return {
        ...
        "new_shape_id": int(new_shape.Id),
        "new_shape_name": new_shape.Name,
        "new_shape_index": new_shape.ZOrderPosition,
    }


def _set_zorder_impl(slide_index, shape_id, shape_name, shape_index, z_order_cmd):
    app, pres, slide = get_slide(slide_index)
    shape = _get_shape(
        slide, None, shape_name=shape_name, shape_index=shape_index, shape_id=shape_id
    )
...
def add_picture(params: AddPictureInput) -> str:
    ...
    """
    return execute_json('Failed to add picture: ',
            _add_picture_impl,
...
def add_line(params: AddLineInput) -> str:
    ...
    """
    return execute_json('Failed to add line: ',
            _add_line_impl,
            params.slide_index,
...
def list_shapes(params: ListShapesInput) -> str:
    ...
    """
    return execute_json('Failed to list shapes: ', _list_shapes_impl, params.slide_index)
...
def get_shape_info(params: ShapeIdentifierInput) -> str:
    ...
    Args:
        params: Slide index and shape identifier (stable ID, name, or index).

    Returns:
        JSON with detailed shape properties.
    """
    return execute_json('Failed to get shape info: ',
            _get_shape_info_impl,
            params.slide_index, params.shape_id, params.shape_name, params.shape_index,
        )
...
def update_shape(params: UpdateShapeInput) -> str:
    ...
    """
    return execute_json('Failed to update shape: ',
            _update_shape_impl,
            params.slide_index, params.shape_id, params.shape_name, params.shape_index,
            params.left, params.top, params.width, params.height,
...
def delete_shape(params: ShapeIdentifierInput) -> str:
    ...
    """
    return execute_json('Failed to delete shape: ',
            _delete_shape_impl,
            params.slide_index, params.shape_id, params.shape_name, params.shape_index,
        )
...
def duplicate_shape(params: ShapeIdentifierInput) -> str:
    ...
    """
    return execute_json('Failed to duplicate shape: ',
