...
_CONFIGURED_MODELS: set[type[BaseModel]] = set()
...
def _compact_json_schema(schema: dict[str, Any]) -> None:
    """Drop schema labels that repeat field names or tool descriptions."""
...
def make_strict_input_models(func: Callable[..., Any]) -> None:
    """Forbid extra fields and compact generated tool schemas."""
    try:
    ...
    while pending:
        ...
        if not inspect.isclass(annotation) or not issubclass(annotation, BaseModel):
            continue
        if annotation in seen or annotation in _CONFIGURED_MODELS:
            continue
        seen.add(annotation)
    ...
    for model in models:
        model.model_config = ConfigDict(
            **{
                **model.model_config,
                "extra": "forbid",
                "json_schema_extra": _compact_json_schema,
            }
        )
    for model in reversed(models):
        model.model_rebuild(force=True)
    _CONFIGURED_MODELS.update(models)
