...
- **28 カテゴリ・165 ツール** — プレゼンテーション、スライド、図形、テキスト、表、グラフ、テーマ、アニメーション、メディア、デザイン参照、エクスポートなどを扱えます。
...
- **ネイティブ検証** — 図形スナップショットの差分と保存前検証を利用できます。
...
1. `ppt_activate_presentation` を呼び出し、`ppt_get_presentation_info` と `ppt_list_shapes` で対象を確認します。
...
作業中は、計画、ツール呼び出しとその結果、進捗などの中間情報を
...
| デザイン参照 | 1 | React Bits を参考にした PowerPoint ネイティブのデザインレシピと公開リンク |
| **合計** | **165** | |
